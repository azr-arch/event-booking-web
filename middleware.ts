...
export default auth(async (req) => {
    ...
    const isStaticFile = nextUrl.pathname.match(/\.(png|jpg|jpeg|svg|css|js|ico|woff2|ttf)$/);

    console.log(
        `Request: ${nextUrl.pathname}, Authenticated: ${!!token}, Role: ${token?.role || "None"}`
    );
    ...
    if (token) {
        ...
        if (isAdminRoute || isClientProtectedRoute) {
            ...
            return NextResponse.redirect(new URL(`/sign-in?callbackUrl=${callbackRoute}`, req.url));
        }
    }
